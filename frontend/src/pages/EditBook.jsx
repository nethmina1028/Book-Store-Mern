...
const EditBook =() =>{
  ...
  useEffect(() =>{
    ...
    axios.get(`http://localhost:5555/books/${id}`)
      .then((respose) =>{
         setAuthor(respose.data.data.author);
         setPublishYear(respose.data.data.publishYear);
          setTitle(respose.data.data.title);
          setLoading(false);
      }).catch((error)=>{
        setLoading(false);
  ...
  return (
    <div className='p-4'>
      ...
      {loading ? <Spinner /> : ''}
      <div className='flex-col border-2 rounded-xl w-[600px] border-sky-400 p-4 mx-auto'>
